...
mod test
{
    ...
    fn synthetic(width: usize, height: usize) -> Vec<u8> //DETERMINISTIC IMAGE WITH EVERY KIND OF EDGE IN IT
    {
        ...
        for y in 0..height
        {
            for x in 0..width
            {
                //GRADIENTS COVER THE RANGE, THE XOR PATTERN PUTS HARD 1-PIXEL EDGES NEXT TO EACH
                //OTHER SO A WRONG CHROMA AVERAGE CANNOT HIDE IN A SMOOTH IMAGE
    ...
    fn deviation(gpu: &[u8], cpu: &[u8]) -> (u8, f64) //WORST AND MEAN ABSOLUTE DIFFERENCE
    {
        ...
        (worst, total as f64 / gpu.len() as f64)
    }

    #[test]
    fn gpu_conversion_matches_the_cpu_path()
    {
        let (width, height) = (256usize, 128usize);

        //NO GPU IN THIS ENVIRONMENT IS NOT A TEST FAILURE - IT IS THE CASE THE FALLBACK EXISTS FOR
        let Ok(mut converter) = GpuConverter::new() else { return; };

        let rgba = synthetic(width, height);

        let mut expected = YUVBuffer::new(width, height);
        expected.read_rgb(RgbaSliceU8::new(&rgba, (width, height)));

        let actual = converter.convert(width as u32, height as u32, &rgba)
            .expect("the converter must accept a supported resolution");

        assert_eq!(actual.dimensions(), expected.dimensions());
        assert_eq!(actual.strides(), expected.strides());

        //THE CPU PATH FOR *RGBA* IS openh264's FLOAT `write_yuv_by_pixel`, WHICH TRUNCATES AND
        //AVERAGES CHROMA WITHOUT ROUNDING; THE SHADER USES THE INTEGER COEFFICIENTS AND THE +2
        //ROUNDING OF openh264's OWN `write_yuv_scalar`. THE TWO AGREE TO WITHIN A LEAST
        //SIGNIFICANT BIT, WHICH IS WHY THIS IS A TOLERANCE AND NOT AN EQUALITY.
        let (worst_y, mean_y) = deviation(actual.y(), expected.y());
        assert!(worst_y <= 1, "luma differs by {worst_y} (mean {mean_y:.4})");
        assert!(mean_y < 0.6, "luma mean deviation {mean_y:.4} is too high");

        let (worst_u, mean_u) = deviation(actual.u(), expected.u());
        assert!(worst_u <= 2, "u differs by {worst_u} (mean {mean_u:.4})");

        let (worst_v, mean_v) = deviation(actual.v(), expected.v());
        assert!(worst_v <= 2, "v differs by {worst_v} (mean {mean_v:.4})");
    }

    #[test]
    fn unsupported_resolutions_are_refused_rather_than_packed_wrong()
    {
        //WIDTH NOT A MULTIPLE OF 8 CANNOT BE PACKED FOUR CHROMA SAMPLES TO A WORD
        assert!(!GpuConverter::supports(1366, 768));
        assert!(!GpuConverter::supports(1920, 1081));
        assert!(!GpuConverter::supports(0, 0));

        assert!(GpuConverter::supports(1920, 1080));
        assert!(GpuConverter::supports(1600, 900));
        assert!(GpuConverter::supports(3840, 2160));
    }
}
