...
pub enum Connection //CLIENT CONNECTION (WHAT IS PUSHED TO connections LIST)
{
    Authenticated
    {
        ...
        screen_download_stream: Option<Arc<Mutex<TcpStream>>>, //SCREEN UPLOAD STREAM
...
impl Connection
{
    ...
    pub fn screen_download_stream(&self) -> &Option<Arc<Mutex<TcpStream>>>
    {
        match self
        {
            Self::Authenticated { screen_download_stream, .. } => screen_download_stream,
            Self::NonAuthenticated { .. } => &None,
        }
    }
...
fn update_client_keys(peer_addr: &SocketAddr, keys: &SharedKeys) //ADD KEY TO NonAuthenticated CLIENT AFTER KEY EXCHANGE
{
    ...
    CONNECTIONS.alter(peer_addr, |_, old_connection|
    {
        match old_connection
        {
            ...
            Connection::Authenticated { write_stream, file_streams, screen_upload_stream, screen_download_stream, username, id, last_activity, channel,
...
fn update_client_channel(peer_addr: &SocketAddr, channel: &Option<String>) //MOVE CLIENT TO CHANNEL
{
    ...
    CONNECTIONS.alter(&peer_addr, |_, old_connection|
    {
        Connection::Authenticated
        {
            ...
            screen_download_stream: old_connection.screen_download_stream().clone(),
            peer_addr: *old_connection.peer_addr(),
