...
fn test_input_diffusion()
{
    ...
    for _ in 0..iterations
    {
        ...
        let base_encrypted = encrypter::encrypt::<W, H>(input.clone(), Some(key.clone()))
        ...
        for i in 0..input.len()
        {
            for bit in 0..64
            {
                ...
                let modified_encrypted = encrypter::encrypt::<W, H>(modified_input, Some(key.clone()))
                    .expect("Encryption failed")
                    .output;
    ...
    for _ in 0..iterations
    {
        ...
        for i in 0..key.len()
        {
            for bit in 0..64
            {
                ...
                let modified_encrypted = encrypter::encrypt::<W, H>(input.clone(), Some(modified_key))
                    .expect("Encryption failed")
                    .output;
